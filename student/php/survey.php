...
// Fetch questions with 'major' category and 'rating' type for the specific survey
$question_query = mysqli_query($conn, "SELECT * FROM questions WHERE survey_id = '$survey_id' AND category = 'major' AND question_type = 'rating'") or die('Question query failed');
$questions = [];
...
while ($question = mysqli_fetch_assoc($question_query)) {
    ...
    $rate_query = mysqli_query($conn, "SELECT * FROM rate WHERE question_id = '$question_id'") or die('Rating options query failed');
    $rate_data = mysqli_fetch_assoc($rate_query);

    if ($rate_data) {
        for ($i = 1; $i <= 5; $i++) {
            if (isset($rate_data[(string)$i])) {
                $options[] = $rate_data[(string)$i];
            }
        }
    } else {
    ...
    <div class="container">
        <form action="survey2.php" method="POST">
                ...
                <div class="question-section" id="question-<?php echo $question['question_id']; ?>">
                    ...
                    <?php foreach ($question['options'] as $index => $option): ?>
                            ...
                            <input type="radio" name="responses[<?php echo $question['question_id']; ?>]"
                                ...
                                <?php if (isset($_SESSION['survey_responses'][$user_id][$survey_id][$question['question_id']]) 
                                            && $_SESSION['survey_responses'][$user_id][$survey_id][$question['question_id']] == $index + 1) {
                                            echo 'checked';
                                        } ?>>
            ...
            <div class="navigation1">
                <button type="submit" formaction="survey2.php" class="nav-button next-button">Next</button>
            </div>
        </form>
    </div>
</main>
...
<script>
     document.querySelector('form').addEventListener('submit', function (event) {
        ...
        questions.forEach(question => {
            ...
            if (!answered) {
                allAnswered = false;
                question.classList.add('unanswered'); // Highlight unanswered question
            } else {
